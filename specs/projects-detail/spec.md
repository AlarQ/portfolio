...
`ds/RoadmapSection` with heading `id="roadmap"` (`<h2>`, eyebrow `ROADMAP`; legend line under the heading on mobile). Layout **A**: one ordered list in authored order - `toward` Features, then a horizontal Milestone divider - a centred **pill chip** (`rounded-pill`, primary border/text, as in the #102 prototype) labelled `<Milestone> · <shipped> of <total> shipped`, flanked by hairline rules (optional success-tone divider at 100%), a muted `After <Milestone>` caption, then `beyond` Features. Each row: `status-dot` (tone from FR-4) + Feature name; the status word lives in the row's **accessible name** (e.g. `sr-only` text), no visible per-row status text. **No second meter here.** No two-column/board layouts. Component receives pre-grouped Features (FR-3) and renders `milestoneName` directly. **Accepted deviation:** implemented as **two separate `<ol>` elements** (toward, then beyond) rather than one continuous list, so a screen reader announces each list's correct length and the divider/caption sit outside both lists as grouping semantics, instead of "one ordered list" being taken literally as a single `<ol>`.
