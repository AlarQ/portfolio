...
- If the post uses `<Diagram>`, author each `content/diagrams/<name>.excalidraw` scene (or invoke the `excalidraw-diagram` skill) and run `pnpm prerender:diagrams` so the SVGs exist.
