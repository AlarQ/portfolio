 ...
 *
 * `content/posts/` now holds two real Posts - `pour-it-once.mdx`
 * (2026-07-14, newest) and `my-spec-driven-workflow.mdx` (2026-06-15, oldest) - so
 * this spec exercises the multi-Post *boundary* scenario against real content: the
 * newest Post links only to its older neighbour, the oldest only to its newer
...
test.describe("Blog Post navigation (multi-Post boundary)", () => {
  test("newest Post links to its older neighbour only (Older, no Newer)", async ({ page }) => {
    const response = await page.goto("/blog/pour-it-once");
    expect(response?.status()).toBe(200);

    const nav = page.getByRole("navigation", { name: "Post navigation" });
    await expect(nav).toBeVisible();

    // The older neighbour (my-spec-driven-workflow) is reachable via an "Older
    // post" link; the newest Post has no newer neighbour.
  ...
  test("oldest Post links to its newer neighbour only (Newer, no Older)", async ({ page }) => {
    ...
    // The newer neighbour (pour-it-once) is reachable via a "Newer post" link;
