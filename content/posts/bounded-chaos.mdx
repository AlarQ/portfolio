...
- A **findings report** per task and gate holds what the gate found. Each finding
  carries a severity, a category, a source of tool or model, a confidence, and a
...
<Diagram name="learning-loop" alt="The learning loop: the model's work is recorded to an event log and findings reports; those go to me to accept or reject; the reviewed findings are then mined; the mining promotes new rules; and those rules feed back into the model's next run so the same mistake is caught earlier." />
