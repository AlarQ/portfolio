...
<Diagram name="feature-flow" alt="The feature flow: a fixed Plan → Build → Verify → Ship chain, read top to bottom. Plan holds explore then propose; Build holds implement; Verify holds validate, review-and-ship, then learn-from-reports; Ship holds validate-impl. Each step names its phase, and shipping is an inline step on the validate and review-and-ship paths." />
