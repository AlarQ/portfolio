...
flowchart TB
  ...
  subgraph s_verify["Verify"]
    ...
    review["<b>review-findings</b><br/>rule on the findings"]
    learn["<b>learn-from-reports</b><br/>never the same mistake twice"]
  end
...
```mermaid
graph LR
  acts["LLM actions"] --> log[".monitor.jsonl<br/>reports/*.yaml"]
  log --> review["review-findings<br/>accept / reject"]
  review --> learn["learn-from-reports"]
  learn --> kb["KB rules"]
  kb -->|caught earlier next run| acts

  classDef loopNode fill:#241033,stroke:#a855f7,stroke-width:2px,color:#e2e8f0;
  class acts,log,review,learn,kb loopNode;
```
...
## I touch it constantly, but decide rarely

This resolves a tension in the flow. Nothing auto-advances, so I'm hands-on at
every phase. Yet the whole point is to keep the human out of the model's way. Both
hold once you separate two different things I'm doing.

Running a phase is a keystroke. It's mechanical, it happens at every step, and it
keeps me oriented: I always know which wall the work is behind right now. That
constant contact is cheap and carries no judgment.

Judgment lives at three gates, and they're the three human stops above. First,
after `propose`, I read the spec and the task breakdown before any code exists.
Second, after `implement`, I read the draft PR to see what the model actually
built. Third, after `validate-impl`, I review the whole feature branch before it
reaches `main`. Between those stops, the vessel just runs.

That split is the whole point. The system does the typing and the ordering; I
weigh in only where perspectives clash and someone has to make the call. Lots of
keystrokes, very few real decisions — which is exactly the trade I want.
...
scratch for [Pi](https://pi.dev), a minimal agent harness. A read-only Dashboard
for watching feature state as it runs is planned too. The point is that the
