...
<Diagram name="feature-flow" alt="The feature flow: a fixed Plan → Build → Verify → Ship chain of four stages. Plan holds explore then propose; Build holds implement; Verify holds validate, review-and-ship, then learn-from-reports; Ship holds validate-impl. Shipping is an inline step on the validate and review-and-ship paths. The stages run left to right in that order." />

Each command owns one phase of the flow. Here's what every phase is *for*:

### `explore` - capture requirements, size the work

This phase runs a relentless ["grill" interview](https://github.com/mattpocock/skills/tree/main/skills/engineering/grill-with-docs) to pin down intent. It then grounds that intent in the domain: it updates `CONTEXT.md` - the project's living domain glossary, the canonical vocabulary the model has to speak - and writes ADRs for the hard-to-reverse, surprising calls. Finally it infers the **tier** - how much ceremony the feature earns - and the rest of the feature config once, up front (the full set of dials is spelled out in *The spec config* below).
...
This phase builds exactly one eligible task, on its own branch, through a red-green-refactor loop; the next eligible task is picked off the **dependency graph** - the build order already set in `propose`. It runs inline or hands off to a specialized implementer agent, driving the task `todo → in-progress → implemented` and opening a draft PR.
...
validate_scope: per-task # per-task | per-spec | both  - when the gates run
...
<Diagram name="validate-panel" alt="The validate phase runs three tracks in parallel, all feeding review-and-ship. Track 1, deterministic gates: lint, type-check, tests, format. Track 2, advisory agents: Security Engineer, Code-Quality Pragmatist, Software Architect. Track 3, coverage audit: the coverage auditor checking the diff against acceptance criteria. Every node flows into review-and-ship." />
...
quality, architecture), and a coverage auditor checks it against the task's acceptance
...
- A running log tracks each feature's lifecycle: task transitions, tests going
  red then green, when coverage checks start and finish, gates skipped, the
  tier the feature was assigned, and the PR's journey from draft to ready.
- Every task keeps a record of its gate findings - what was flagged, how
  severe, whether a tool or an agent raised it, how confident it was, and
  whether I accepted, rejected, or noted it.
- Once a feature is fully built, an audit report says whether it's genuinely
  complete or needs another pass.

On top of that sits a learning loop: recurring findings and reasoned rejections
across past runs get mined for patterns general enough to reuse, and those
patterns turn into knowledge-base rules. The next run starts with the rule
already in place, so the same mistake gets caught one phase earlier.
