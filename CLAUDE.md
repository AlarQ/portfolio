...
Every diagram is authored as a role-tagged spec in `content/diagrams/*.diagram.ts`, built with Excalidraw's builder API. All diagrams draw from the **one** palette owned by `scripts/diagram-lib/palette.ts` - category Badge hues in light, Shiki token colors in dark, resolved per-theme from `src/theme/tokens.ts` primitives (same seam pattern as Design tokens above). A `.diagram.ts` spec declares structure + a semantic role per node (`plan` | `build` | `verify`, etc.) and never a color literal. Do not introduce a second palette, inline color literals, or a one-off style for a single diagram (including Project Briefs) - add a new role to the palette in `scripts/diagram-lib/palette.ts` if an existing hue doesn't fit, then re-run `pnpm prerender:diagrams`. Render with `<Diagram name="..." alt="..." />` (`src/components/Diagram.tsx`, unchanged).
