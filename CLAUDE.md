...
Every diagram is a native `content/diagrams/<name>.excalidraw` JSON scene - hand-editable in any Excalidraw client (excalidraw.com, the VS Code extension, Obsidian) and LLM-editable as JSON. Diagrams are authored in the **light** palette only; the dark SVG is derived at render time by `scripts/diagram-lib/theme.ts`'s exhaustive hex swap (light→dark maps owned by `scripts/diagram-lib/palette.ts`). An off-palette colour is a **render failure** (a thrown error), never a silent pass. Copy shapes from `content/diagrams/_palette.excalidraw` rather than picking a colour freehand in the Excalidraw UI - a hand-picked hex will not be in the swap map and will fail the render. Regenerate all SVGs with `pnpm prerender:diagrams`. Render with `<Diagram name="..." alt="..." />` (`src/components/Diagram.tsx`, unchanged).
...
- **Project data lives under `src/data/projects/`**: `index.ts` carries the types + hand-ordered `projects` array + `getProjects()`/`getProject(slug)`, and one module per Project (`hyperion.ts`, `bondsmith.ts`) - never a parallel per-concept split file (e.g. no `projectRoadmaps.ts` keyed by slug; that is the shape `domains.ts` exists to prevent). `src/data/roadmap.ts` derives `MilestoneProgress` from a `Roadmap` - never authored, never computed in a component.
- **Capability media URLs** resolve through `src/data/mediaConfig.ts` (`getMediaBaseUrl()`, env `MEDIA_BASE_URL`, server-only, baked at SSG - falls back to `/media` when unset) and the `resolveClipUrl`/`resolvePosterUrl`/`resolveCapabilityMedia` seam functions in `src/utils/projectPresentation.tsx`. Clips are addressed off `mediaBaseUrl` (e.g. Vercel Blob); posters/stills are always on-site under `public/media/<slug>/`, committed to the repo.
- Media authoring has a documented budget (clip length/size/codec, poster size) - see spec `specs/projects-detail/spec.md` FR-12 before adding a clip or still.
