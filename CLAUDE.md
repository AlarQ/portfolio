...
Every diagram is a `content/diagrams/<name>.d2` source rendered by the `@terrastruct/d2` WASM build (ADR-0005) - no browser, no network. A source carries **structure + semantic classes only, never a colour**; the look lives in a prelude that resolves `src/theme/tokens.ts` primitives by name (`scripts/diagram-lib/theme.ts`), and one source is rendered once per theme to `public/diagrams/<name>-{light,dark}.svg`. Three render gates fail the commit: **a colour literal anywhere in a `.d2`**, **a class outside the closed vocabulary** (`node`, `emphasis`, `terminal`, `group`, the `planned`/`stepper` modifiers, `edge`, `edge-feedback` - D2 ignores an unknown class silently, so a typo would render a plausible-looking unstyled box), and **an aspect ratio wider than 2:1** (illegible in the ~640px prose column; flow chains down with `direction: down` + the `stepper` modifier). `content/diagrams/_reference.d2` is the exemplar board exercising the whole vocabulary - `_`-prefixed sources are exempt from the aspect gate only and can never be embedded. Regenerate all SVGs with `pnpm prerender:diagrams` (also run by the pre-commit hook; `src/theme/diagrams.test.ts` re-renders and byte-compares, so a stale SVG fails pre-push). Render with `<Diagram name="..." alt="..." />` (`src/components/Diagram.tsx`, unchanged). Invoke the `d2-diagram` skill for the authoring craft.
